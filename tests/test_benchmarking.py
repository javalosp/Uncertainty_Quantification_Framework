...
class TestBenchmarkingOrchestrator(unittest.TestCase):
    """
    Automated verification suite for the CRPS & Coverage Probability Benchmarking Orchestrator[cite: 15].
    Verifies metric equations, model registration across Step 1 and Step 2, and ranking summaries[cite: 15].
    """
    @classmethod
    def setUpClass(cls):
        """Generate synthetic time series with train/test split[cite: 15]."""
        np.random.seed(42)
        ...
        # Train on 2000-2019 (20 years), test on 2020-2025 (6 years)[cite: 15]
    ...
    def test_01_probabilistic_metrics(self):
        """Verify exact calculations for Gaussian CRPS, empirical CRPS, PICP, and MIW[cite: 15]."""
        ...
        self.assertIn('CRPS', metrics, "CRPS metric missing from summary[cite: 15].")
        self.assertIn('PICP_95%', metrics, "Coverage probability missing from summary[cite: 15].")
        self.assertIn('Sharpness_MIW', metrics, "Sharpness MIW missing from summary[cite: 15].")
        self.assertGreater(metrics['CRPS'], 0.0, "CRPS must be strictly positive for imperfect predictions[cite: 15].")
        self.assertEqual(metrics['PICP_95%'], 1.0, "All true values fall within 2 sigma; PICP should be 1.0[cite: 15].")
    ...
    def test_02_orchestrator_execution_and_ranking(self):
        """Verify backtesting execution across Step 1 and Step 2 models and comparative plotting[cite: 15]."""
        ...
        # Register Step 1 Models[cite: 15]
        ...
        # Register Step 2 Models[cite: 15]
        ...
        # Run benchmark[cite: 15]
        df_results = orchestrator.run_benchmark()
        
        self.assertIsInstance(df_results, pd.DataFrame, "Orchestrator must return a Pandas DataFrame[cite: 15].")
        self.assertEqual(len(df_results), 5, "All 5 registered models must be evaluated[cite: 15].")
        
        # Verify ranking order (CRPS ascending)[cite: 15]
        crps_vals = df_results['CRPS'].values
        self.assertTrue(np.all(np.diff(crps_vals) >= 0), "Results DataFrame must be sorted by CRPS in ascending order[cite: 15].")
        
        # Test comparative visualization export[cite: 15]
        plot_path = "test_benchmark_comparison.png"
        orchestrator.plot_benchmark(save_path=plot_path)
        self.assertTrue(os.path.exists(plot_path), "Benchmark plot image was not created[cite: 15].")
