...
class EmpiricalDataProcessor:
    """
    Phase 1: Architectural Setup for Dynamic Preprocessing.
    
    Responsibility: Acts as the bridge between raw, messy historical datasets 
    and the strict, structured input required by the DynamicDataManager. 
    It replaces subjective human Pedigree scoring with algorithmic data evaluation.
    """
    def __init__(self, raw_historical_data, proxy_data=None, simulation_start_year=2024, simulation_end_year=2050):
        """
        Args:
            raw_historical_data (pd.DataFrame): Time-series data of historical flows.
                Expected format: Rows = Years, Columns = Flow_Names.
            proxy_data (pd.DataFrame): Exogenous variables (e.g., GDP, Population) for forecasting.
            simulation_start_year (int): The baseline year (t=0) for the dynamic simulation.
        """
        self.raw_data = raw_historical_data.copy()
        self.proxy_data = proxy_data.copy() if proxy_data is not None else None
    ...
    # ALGORITHMIC DATA QUALITY INDICATORS (DQIs)
    # -------------------------------------------------------------------------
    def _calculate_empirical_dqis(self, flow_series):
        ...
        # 4. Aleatory Baseline (Calculating Empirical GSD)
        ...
        if mean_val > 0 and std_val > 0:
            ...
            # Formula: GSD = exp(sqrt(ln(1 + CV^2)))
            gsd = np.exp(np.sqrt(np.log(1 + cv**2)))
            
            # 5. Reliability Penalty based on excessive noise
            rel_score = 5 if cv > 1.0 else (3 if cv > 0.5 else 1)
        else:
    ...
    # IMPUTATION & EPISTEMIC EXTRACTION (GAUSSIAN PROCESSES)
    # -------------------------------------------------------------------------
    def _impute_with_gaussian_process(self, flow_series):
        ...
        if len(valid_data) < 3:
            default_mean = valid_data.mean() if not valid_data.empty else 0.0
        ...
        # Define the Kernel: 
        # RBF (Smooth non-linear trends) + WhiteKernel (Accounts for historical measurement noise)
    ...
    # STRUCTURAL UNCERTAINTY VIA PROXY ENSEMBLES
    # -------------------------------------------------------------------------
    
    # 1. Define the competing mathematical models
    @staticmethod
    ...
    def _evaluate_proxy_ensembles(self, flow_series, proxy_name):
        """
        Phase 4 Upgrade: Uses an ensemble of machine learning models (Linear, Ridge, 
        and Decision Tree) to map historical flows to macroeconomic proxies.
        The spread between these algorithms forms the structural epistemic envelope.
        """
        # 1. Fallback check: If proxy data is missing, revert to Gaussian Process
        if self.proxy_data is None or proxy_name not in self.proxy_data.columns:
            ...
            # GP returns 4 variables, but orchestrator expects 3 from proxy, so we slice [:3]
            gp_mu, _, gp_min, gp_max = self._impute_with_gaussian_process(flow_series)
            return gp_mu, gp_min, gp_max

        # 2. Align historical data to train the models
        valid_data = flow_series.dropna()
        ...
        # 3. Define the Wang-inspired Model Ensemble
        models = {
        ...
        # 4. Train all models and generate competing future forecasts
        ...
        if not predictions:
            # Absolute fallback if all ML models violently crash
            gp_mu, _, gp_min, gp_max = self._impute_with_gaussian_process(flow_series)
            return gp_mu, gp_min, gp_max

        # 5. The P-Box Synthesis: Extract Structural Ignorance
    ...
    def _evaluate_proxy_ensembles_old(self, flow_series, proxy_name):
        """
        OLD VERSION OF THE FUNCTION USING REGRESSIONS
        Fits multiple economic models (Linear, Exponential, Logistic) using a proxy 
        variable (e.g., GDP) to quantify Model Structure Uncertainty.
        ...
        if not predictions:
            # Fallback if all models fail to converge
    ...
    # FUZZY DELAY DISTRIBUTIONS (LIFETIMES)
    # -------------------------------------------------------------------------
    ...
    def _calculate_fuzzy_delay_distribution(self, min_life, mode_life, max_life, std_dev):
        ...
        def _get_fractions(mean_L):
            ...
            # Normalize to ensure exactly 100% of the cohort eventually leaves
    ...
    def generate_structured_parameters(self, flow_mapping_dict=None, proxy_mapping_dict=None, 
        ...
        """
        Executes the preprocessing pipeline for every flow.
        NEW: Accepts topology_mapping_dict (e.g., {'Cu_Mining': {'Source': 'Environment', 'Target': 'Refining'}})
        """
        """
        Executes the preprocessing pipeline for every flow.
        Now supports delay_mapping_dict (e.g., {'Copper_Wire': {'min': 25, 'mode': 30, 'max': 35, 'std': 5}})
        """
        """
        Executes the preprocessing pipeline for every flow.
        Now supports proxy_mapping_dict (e.g., {'Copper_Demand': 'Global_GDP'})
        """
        print(f"[*] Initializing Empirical Preprocessing for {len(self.raw_data.columns)} flows...")
        ...
        for flow_name in self.raw_data.columns:
            ...
            mapped_proxy = proxy_mapping_dict.get(flow_name, None) if proxy_mapping_dict else None

            if mapped_proxy is not None and self.proxy_data is not None:
                # --- PHASE 4 INTEGRATION: Proxy Ensemble ---
        ...
        # VISUALISATION INTEGRATION: Extract Topology 
            # ====================================================================
